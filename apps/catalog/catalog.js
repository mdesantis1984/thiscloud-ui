const componentManifest = __TC_COMPONENT_MANIFEST__;

const formFamily = componentManifest.families.find(({ id }) => id === 'inputs-forms');
if (!formFamily) throw new Error('Inputs/Forms component contract is missing.');
const categories = {
  'Foundations/Layout': ['BreakpointProvider','Container','Divider','Element','FocusTrap','Grid','Hidden','Highlighter','Image','Link','Paper','ScrollToTop','Skeleton','SwipeArea','Typography'],
  Actions: ['Button','ButtonFab','ButtonGroup','IconButton','ToggleIconButton','Toolbar'],
  'Inputs/Forms': formFamily.components.map(({ name }) => name),
  'Navigation/Disclosure': ['AppBar','Breadcrumbs','Carousel','Drawer','ExpansionPanels','List','Menu','NavMenu','Pagination','Tabs','TreeView'],
  'Surfaces/Content': ['Avatar','Badge','Card','Icons','Rating'],
  'Feedback/Overlays': ['Alert','Dialog','MessageBox','Overlay','Popover','Progress','Snackbar','Tooltip'],
  'Data/Visualization': ['BarChart','DataGrid','DonutChart','LineChart','PieChart','SimpleTable','StackedBarChart','Table','Timeline'],
};
const production = new Set([
  'BreakpointProvider','Container','Divider','Grid','Image','Link','Paper','Typography','Button','ButtonFab','ButtonGroup','IconButton','Toolbar',
  'AppBar','Breadcrumbs','Drawer','List','Menu','Pagination','Tabs','Avatar','Badge','Card','Alert','Dialog','Overlay','Popover','Progress','Snackbar','Tooltip','DataGrid','Table','SimpleTable','Timeline',
  ...formFamily.components.filter(({ support }) => support.web === 'release-candidate').map(({ name }) => name),
]);
...
const publicWebApi=Object.freeze(Object.fromEntries(formFamily.components
  .filter(({ support }) => support.web === 'release-candidate')
  .map(({ name, web }) => [name, { identity: web.identity, boundary: web.boundary, code: web.example }])));all.forEach(record=>{record.api=publicWebApi[record.name];record.status=record.api?'rcApi':'demoOnly'});const routeStatus=name=>itemFor(name)?.status||'demoOnly',apiBoundary=api=>`<span class="pill info">${esc(api.identity)}</span><span class="pill">${esc(api.boundary)}</span>`,demoBoundary=()=>`<span class="pill demoOnly">${t('component.demoOnly')}</span>`;
...
function radioDemo(variant) {
  ...
  const options = copy.options.map((option, index) => `<tc-radio class="radio-option"
    id="${id}-${index}" name="${id}-group" value="${index}" label="${esc(option)}"
    ${index === selected ? ' checked' : ''}${variant === 'states' && index === 0 ? ` required required-message="${esc(copy.requiredMessage)}"` : ''}
    ${variant === 'states' && index === 2 ? ' disabled' : ''}></tc-radio>`).join('');
...
function wireRadioDemo() {
  document.querySelectorAll('[data-radio-demo]').forEach(root => {
    ...
    root.querySelectorAll('tc-radio').forEach(input => input.onchange = () => {
      const selected = [...root.querySelectorAll('tc-radio')].find((control) => control.checked);
      status.textContent = selected ? `${copy.selected} ${selected.label}` : copy.none;
    });
  });
}
...
function nativeInputDemo(name, variant) {
  ...
  if (name === 'Checkbox') {
    const control = (suffix, label, attributes = '') => `<tc-checkbox class="boolean-option" id="${id}-${suffix}"
      name="${id}" value="${suffix}" label="${esc(label)}"${attributes}></tc-checkbox>`;
    const options = variant === 'overview'
      ? control('checked', copy.checked, ' checked') + control('unchecked', copy.unchecked)
      : variant === 'variants'
        ? control('checked', copy.checked, ' checked') + control('indeterminate', copy.indeterminate, ' data-indeterminate')
        : control('required', copy.required, ` required required-message="${esc(copy.requiredMessage)}"`)
          + control('disabled', copy.disabled, ' disabled');
...
function wireNativeInputs() {
  document.querySelectorAll('[data-native-input], [data-specialized-input]').forEach(root => {
    ...
    const copy = t('inputForms')[name[0].toLowerCase() + name.slice(1)];
    root.querySelectorAll('[data-indeterminate]').forEach(control => { control.indeterminate = true; });
    ...
    if (name === 'Checkbox') root.querySelectorAll('tc-checkbox').forEach(control => control.onchange = () => {
      status.textContent = `${control.label}: ${control.checked ? copy.on : copy.off}`;
    });
...
function categoryPage(v){const c=Object.keys(categories).find(x=>catSlug(x)===v);if(!c)return notFound();return `<div class="page-head"><div class="eyebrow">${t('category.eyebrow')}</div><h1>${t('nav.categories.'+c)}</h1><p>${categories[c].length} ${t('category.intro')}</p></div><div class="component-grid">${categories[c].map(n=>`<article class="card"><div class="schematic">${demo(n,'overview')}</div><div class="card-body"><h3>${n}</h3><p>${esc(t('components.'+n))}</p>${badge(routeStatus(n))}<div class="actions"><a class="btn primary" href="#component/${itemFor(n.replace(/([a-z])([A-Z])/g,'$1-$2').toLowerCase()).slug}">${t('category.openReference')}</a></div></div></article>`).join('')}</div>`;}
...
function downloadPage(){const d=t('download'),file='thiscloud-ui-web-0.1.0-rc.5.tgz';return `<div class="page-head download-guide"><div class="eyebrow">${d.eyebrow}</div><h1>${d.title}</h1><p>${d.intro}</p><div class="actions"><a class="btn primary" data-package-download href="/downloads/${file}" download>${icon('download')}${d.package}</a><a class="btn" data-checksum-download href="/downloads/${file}.sha256" download>${icon('verified')}${d.checksum}</a></div></div><div class="grid download-guide"><article class="card"><div class="card-body"><h2>${d.installTitle}</h2><ol><li><strong>${d.step1Title}</strong><p>${d.step1}</p></li><li><strong>${d.step2Title}</strong><p>${d.step2}</p><h3>${d.linux}</h3><pre class="code"><code>sha256sum -c ${file}.sha256</code></pre><h3>${d.powershell}</h3><pre class="code"><code>Get-FileHash .\\${file} -Algorithm SHA256</code></pre></li><li><strong>${d.step3Title}</strong><pre class="code"><code>pnpm add ./${file}</code></pre></li><li><strong>${d.step4Title}</strong><pre class="code"><code>import '@thiscloud/ui-web';\nimport '@thiscloud/ui-web/tokens.css';</code></pre></li></ol></div></article><article class="card"><div class="card-body"><h2>${d.useTitle}</h2><p>${d.useIntro}</p><pre class="code"><code>&lt;tc-checkbox label="Accept terms" required&gt;&lt;/tc-checkbox&gt;\n&lt;tc-radio name="environment" value="production" label="Production" required&gt;&lt;/tc-radio&gt;\n&lt;tc-switch label="Notifications"&gt;&lt;/tc-switch&gt;\n&lt;tc-text-field label="Organization"&gt;&lt;/tc-text-field&gt;</code></pre><p>${d.boundary}</p><span class="pill production">0.1.0-rc.5</span></div></article></div><div class="actions"><a class="btn" href="/">${icon('arrow_back')}${d.back}</a></div>`;}
