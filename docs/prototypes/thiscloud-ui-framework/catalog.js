...
function chrome(){document.documentElement.lang=locale;document.title=t('meta.title');for(const id of ['skip','brandSupport','workspace','workspaceStrong','workspaceVersion','footerStatus','footerSupport'])document.getElementById(id).textContent=id==='skip'?t('meta.skip'):id==='brandSupport'?t('meta.brandSupport'):id.startsWith('workspace')?t('meta.'+id):id==='footerStatus'?t('meta.rcPreview'):t('meta.footerSupport');const q=document.getElementById('globalSearch');q.setAttribute('aria-label',t('header.searchLabel'));q.placeholder=t('header.searchPlaceholder');document.getElementById('searchHint').textContent=t('header.searchLabel');document.getElementById('menu').setAttribute('aria-label',t('header.openNav'));document.getElementById('theme').setAttribute('aria-label',t('header.toggleTheme'));document.getElementById('profile').setAttribute('aria-label',t('header.profile'));document.getElementById('density').textContent=document.documentElement.dataset.density==='compact'?t('header.comfortable'):t('header.compact');document.getElementById('language').textContent=t('header.language');document.getElementById('language').setAttribute('aria-label',t('header.switchLanguage'));document.getElementById('dialogTitle').textContent=t('dialog.title');document.getElementById('dialogText').textContent=t('dialog.text');document.getElementById('dialogConfirm').textContent=t('dialog.confirm');document.getElementById('dialogCancel').textContent=t('dialog.cancel');document.getElementById('toast').textContent=t('toast.updated');if(location.hash==='#main')history.replaceState(null,'','#explore');nav();render();wireSearch();}
function nav(){const links=[['explore',t('nav.explore'),'explore'],...Object.keys(categories).flatMap(c=>[[catSlug(c),t('nav.categories.'+c),'category'],...categories[c].map(n=>[itemFor(n.replace(/([a-z])([A-Z])/g,'$1-$2').toLowerCase()).slug,n,'component'])]),['getting-started',t('nav.gettingStarted'),'guidance'],['customization',t('nav.customization'),'guidance'],['accessibility',t('nav.accessibility'),'guidance'],['build-status',t('nav.buildStatus'),'guidance'],['icons',t('nav.icons'),'icons']];const groups=[[t('nav.explore'),links.splice(0,1)],[t('nav.components'),links.splice(0,all.length+7)],[t('nav.guidance'),links.splice(0,4)],[t('nav.icons'),links]];document.getElementById('sideNav').innerHTML=groups.map(([h,ls])=>`<section class="nav-section"><div class="nav-label">${esc(h)}</div><nav class="nav" aria-label="${esc(h)}">${ls.map(([s,l,type])=>`<a class="${type==='component'?'component':''}" href="#${type==='category'?'category/':type==='component'?'component/':type==='guidance'?'guidance/':''}${s}">${icon(type==='icons'?'category':type==='category'?'widgets':type==='component'?'radio_button_checked':type==='guidance'?'menu_book':'explore')}<span>${esc(l)}</span></a>`).join('')}</nav></section>`).join('');}
